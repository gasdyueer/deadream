...
const props = withDefaults(
  ...
  { limit: 0, tag: '', sort: 'date', groupByYear: false, detailed: true }
)

const visible = computed<Post[]>(() => {
  const key = props.sort === 'updated' ? 'updated' : 'date'
  const list = props.tag ? posts.filter((post) => post.tags.includes(props.tag as string)) : [...posts]
...
<template>
  <p v-if="!visible.length" class="post-empty">暂无文章。</p>
    ...
    <section v-for="group in groups" :key="group.year" class="post-group">
      ...
      <ul class="post-list post-list-compact">
        <li v-for="post in group.items" :key="post.url">
          <time class="post-date" :datetime="post.date">{{ post.date.slice(5) }}</time>
